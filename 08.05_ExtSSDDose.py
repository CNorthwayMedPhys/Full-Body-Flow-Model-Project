...
#File locations 
dose_folder = "ExtSSD_Files\\4DDoseData\\"
mapping_folder = "ExtSSD_Files\\VOIMappingArray\\"
compartment_folder = "ExtSSD_Files\\CompartmentData\\"
...
class Location (object):
    ...
    def IntVOIMap (self, number,iso): 
        VOIMappingArrayPath = os.path.join(cd,mapping_folder,iso,str(number)+'.npy')
...
class Network (object):
    ...
    def locationIsoInfo(self, iso):
        for location in self.locations:
            ...
            if location.IDnum > 27:    
                location.IntVOIMap (location.IDnum, iso)
    ...
    def runNTDose(self, rxTime, iso): 
        self._currentDoseData = np.load(os.path.join(cd,dose_folder,"Extended"+iso+".npy"))        
...
for BV in nt.BVs:
    BV_data[i] = BV.dose
    i += 1
np.save(os.path.join(cd,"ExtSSD_Results.npy"),BV_data)
