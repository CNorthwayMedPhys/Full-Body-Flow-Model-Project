...
std = 0.232
pdf_fitted = norm.pdf(x, mu, std)
plt.plot(x, pdf_fitted, color = 'blue', label = "Body")

#Added in the torso
# mu = 1.803
# std = 0.172
# pdf_fitted = norm.pdf(x, mu, std)
# plt.plot(x, pdf_fitted, color = 'orange' , label = "Torso")
...
dvhlocation= os.path.join(cd,'RD.CN_XCAT.TBI_2Field.dcm')
...
for j in range(numDVH):
    ...
    if j == 1:
        lstyle = '-.'
    ax.plot(doseArray,volumeArray, label = stNames[j], linestyle = lstyle)
