...
class BloodVolume (object):
    ...
    def doseErr(self):
        """
        Rel Uncert of Dose to the BV
        """
        return self._doseErr
    
    @doseErr.setter
    def doseErr(self,value):
        self._doseErr = value
...
################ Steady State Established ########################
        ...
        current_file = 2
        self._currentDoseData = np.load(os.path.join(cd,dose_filename_AP +"_w" + str(1) +'E.npy'))
...
############# Begin AP Field ###########################
        while localTime < self.Tfield:
            ...
            if self.currentDoseData[2,-1] < localTime and current_file!= 80:
                self._currentDoseData = np.load(os.path.join(cd,dose_filename_AP +"_w" + str(current_file) +'E.npy') )
                current_file += 1
...
###############Patient is ready to be treated ######################            
        ...
        while localTime < self.Tfield:
             ...
             #Check to see if we need to update the dose date
             if self.currentDoseData[2,-1] < localTime and current_file!= 80:
                 self._currentDoseData = np.load(os.path.join(cd,dose_filename_PA +"_w" + str(current_file) +'E.npy') )
                 current_file += 1
...
######## DONE ##############
    ...
    def currentDoseData(self):
         ...
         row = 1 is the voxel index, row = 2 is the time of the event, row =3 is the 
         relative uncert
         """ 
...
results = runSimulation(0) #runs 1E2, three times
