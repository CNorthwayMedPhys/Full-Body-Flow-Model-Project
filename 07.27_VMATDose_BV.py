...
#%%Parameters
dt = 0.0025 #BFS Time step size (s)
dst = 0.025 #Dose sampling time step size (s) Min is 0.025 s
...
#File locations 
dose_folder = "VMAT_Files\\4DDoseData\\"
mapping_folder = "VMAT_Files\\VOIMappingArrays\\"
compartment_folder = "VMAT_Files\\CompartmentData\\"
treatmentplan = "VMAT_Files\\TreatmentPlan.txt"
...
def MUStepSampler(muArray, time):
    timeRange = int(dst/0.025)
    timeIndex = np.where(np.isclose(muArray[0,:],time))[0][0] + 1
    mu = np.sum(muArray[1,timeIndex:timeIndex+timeRange])
    return mu     

def readTreatmentPlan (file):
    with open(file) as x:
        array = x.readlines()
    array = array[1:]
    iso_list = []
    ...
    for i in range(len(array)):
        item = array[i]
        item = item.split(',')
        iso_list.append(item[0])
        time_list.append(float(item[1]))
        MU_list.append(float(item[2]))
        gap_array.append(float(item[3]))
    d = {'iso': iso_list, 'time': time_list, 'MU': MU_list, 'Gap':gap_array}
    df = pd.DataFrame(d)
    return df
...
class Location (object):
    ...
    def IntVOIMap (self, number): 
        VOIMappingArrayPath = os.path.join(cd,mapping_folder,str(number)+'.npy')
        VOIMappingArray = np.load(VOIMappingArrayPath)
        VOIMappingArray[:,0] = VOIMappingArray[:,0] / 1000 #mm -> m
        self._VOIMap = VOIMappingArray
        
    def IntDVH (self,number,iso):
        if number == 11:
            number = 10
        ...
        DVHArrayPath = os.path.join(cd,compartment_folder,iso,str(number)+'DVH.npy')
        DVHArray = np.load(DVHArrayPath)  
        self._DVH = DVHArray
    def IntEventFreq (self,number, iso):
            if number == 11:
                number = 10
            elif number == 13 or number == 14:
                number = 12
            ...
            EventFreqArrayPath = os.path.join(cd,compartment_folder,iso,str(number)+'eventTrace.npy')
...
class BloodVolume (object):
    ...
    def dose_comp(self):
        """
        Dose to the BV while in compartments
        """
    ...
    def vesselMU(self):
        ...
        """
        return self._vesselMU
    
    @vesselMU.setter 
    def vesselMU(self,value):
        self._vesselMU = value
...
class Network (object):
    ...
    def intializeLocations (self):
        ...
        for location in self.locations:
            if location.IDnum > 27:
                ...
                location.IntVOIMap (location.IDnum)
            if location.splittingratiokey != '0':
                location.IntSplittingRatio (SRdf)   
     
    def locationIsoInfo(self, iso):
        for location in self.locations:
            if location.IDnum <= 27:
                location.IntDVH(location.IDnum, iso)
                location.IntEventFreq(location.IDnum, iso)
    ...
    def runNTDry (self, dryTime):
        while self.localtime < dryTime:
    ...
    def runNTDose(self, rxTime, iso): 
        self._currentDoseData = np.load(os.path.join(cd,dose_folder,iso+"_Vessel.npy"))        
        self._currentMUData = np.load(os.path.join(cd,dose_folder,iso+"_MUsteps.npy"))
        
        while self.localtime < rxTime:
            ...
            for BV in self.BVs:
                ...
                if int(self.localtime*10000) % int(dst*10000) == 0:
                    if BV.location > 27:
                        newlocation = self.locations[BV.location]
                        VOIArray = newlocation.VOIMap
                        VOI = VOISelector(BV.distance, VOIArray)
                        doseStep = DoseDataSampler (self._currentDoseData,self.localtime,VOI)
                        absdoseStep = doseStep * 6.6511073E13 #Gy/MU
                        BV._dose_ves = BV.dose_ves + absdoseStep
                        muStep = MUStepSampler(self._currentMUData, self.localtime)
                        BV._vesselMU = BV.vesselMU + muStep #MU
                       ...
                       doseRate = DoseRateSampler(DoseRateArray, self.localtime) #1/s
        ...
        self._localtime = 0
        for BV in self.BVs:
            vesselDose = BV.dose_ves * BV.vesselMU
            BV._dose = BV.dose + vesselDose + BV.dose_comp
            BV._dose_ves = 0 
            BV._dose_comp = 0
            BV._vesselMU = 0
    ...
    def timestep(self):
        ...
        self._Nettime = np.round(self._Nettime, decimals =4) 
        self._localtime += self.dt
        self._localtime = np.round(self._localtime, decimals = 4)
    ...
    def localtime(self):
        """
        Time within the local function
        """
    ...
    @property
    def currentMUData(self):
         """
         """ 
         return self._currentMUData
          
    @currentMUData.setter
    def currentDoseData(self,value):
        self._currentMUData = value        
...
dfTreatmentPlan = readTreatmentPlan(treatmentplan)    

BV_nums = np.arange(4500,1E4+500,500)
results = np.zeros([len(BV_nums),3])

j= 0
for BV_num in BV_nums:

    #Construct and intialize the network
    nt = Network(dt, BV_num)
    nt.setTimes(T, Tss)
    nt.intializeLocations()
    
    #Run until equilbruim is reached
    #print("Finding Equilibrium")
    nt.runNT2Equil()
    print(BV_num)
    #Add in the iso info and run
    for iso_num in range(dfTreatmentPlan.shape[0]):
        iso = dfTreatmentPlan.iso[iso_num]
        nt.locationIsoInfo(iso)
        #print("Running "+iso)
        rxTime = dfTreatmentPlan.time[iso_num]
        nt.runNTDose(rxTime, iso)
        dryTime = dfTreatmentPlan.Gap[iso_num]
        nt.runNTDry(dryTime)
    
    
    BV_data = np.zeros([len(nt.BVs)])
    i = 0    
    for BV in nt.BVs:
        BV_data[i] = BV.dose
        i += 1
    ...
    results[j,2] = BV_num
    j += 1
    np.save(os.path.join(cd,"BVnumResults_VMAT_4thPortion.npy"),results)
      
        
# dsts = np.arange(0.025,1.025,0.025) #Dose sample time step size (s)
# dsts = dsts[::-1]
