...
maxArray=BVsim(np.asarray([1.16222463, 1.21396462, 1.07799579, 0.76160822, 0.65386433, 1.0146046,\
...
#%%
def average_every_nth_value(data_list, n):
    """
    Calculates the average of every nth value in a list.

    Args:
        data_list: The input list of numerical values.
        n: The step interval (e.g., 5 for every 5th value).

    Returns:
        The average of the selected values, or 0 if no values are selected.
    """
    avgArray = []
    selected_values = []
    for i in range(0, len(data_list), n):
        selected_values.append(data_list[i])
        avgArray.append( sum(selected_values) / len(selected_values))
    return avgArray
        
avgArray = average_every_nth_value(maxArray, 5)
xValues = range(0,len(maxArray)+1,5)
plt.plot(xValues, avgArray,'o', c = '0.8' )
...
plt.axvline(x = ToR, color = "black", label = 'Entrance of all BVs')
plt.axhline(y = 1, color = 'black', linestyle = '--', label= "1% Change value")
