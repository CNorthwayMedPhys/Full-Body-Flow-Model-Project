...
vessel_df.at[1,'Radius Values'] = [0.177, 0.17] #cm
...
class Artery(object):
    ...
    def singleVesselImpedance(self, ZL, s_range, r_0):
        ...
        L = r_0 *self.lrr
        A0 = np.pi * (r_0 ** 2)
        Ehr = (self.k[0] *np.exp( self.k[1]*r_0) +self.k[2]) #Youngs Modulus * vessel thickness/radius
        C = (3/2) *(A0)/(Ehr) #complaince
        delta_s = (2 * nu*(gamma +2))/ (rho *r_0**2)
        i = 0
        ...
        for s in s_range:
            if s == 0:
                ...
                try:
                    num = ZL[i] + ((np.tanh(L/d_s, dtype=np.longcomplex))/(s*d_s*C))
                except:
                    num = ZL[i] + (1/(s*d_s*C))
   
                try:
                    denom = s*d_s*C*ZL[i]*np.tanh(L/d_s, dtype=np.longcomplex) + 1
                except:  
                    denom = (s*d_s*C*ZL[i]*1) + 1
...
nu = 0.048 #blood viscosity [cm2 s-1]
R1 = 25300
R2 = 13900
Ct = 1.3384e-6
...
T = 0.917 #[s]
tc = 4 #Normally 4
dt = 0.0025 #normally 1e-5 #[s], #0.0025 from cousins_2014
dx = 0.1 #normally 0.1 #[cm]

q_in = inlet('example_inlet.csv')

Re = 1/nu #[s/cm2]
T = T  # time of one cycle [s]
tc = tc # number of cycles to simulate
dt = dt  # time step size [s]
ntr = 50 # number of time steps to be stored
dx = dx  # spatial step size [cm]
...
k = (k1, k2, k3) # elasticity model parameters (Eh/r) 
out_args = [R1,R2,Ct]#[R1*rc**4/(qc*rho), R2*rc**4/(qc*rho), Ct*rho*qc**2/rc**7] # Windkessel parameters
out_bc = '3wk'
p0 =(85 * 1333.22365)  # zero transmural pressure [g/cm s2] #normal 85
