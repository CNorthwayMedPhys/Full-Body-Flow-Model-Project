...
dosefile = os.path.join(cd,"4DDoseData\Individual Sweeps\TestData\TBI_XCAT_Full_PA_trimmed.3ddose")
...
#Create empty aray
E_array = np.zeros([len(timeArray),numVOI])

ainflu_sum = 0

#%%

voiIndex = 121510 #unstacked
...
finalArray = np.zeroes([np.size(timeArray)+1, np.size(voiIndex)+1]) 
finalArray[1:,0] = timeArray
finalArray[0,1:] = voiArray
finalArray[1:,1:] = doseArray  

print(sum(doseArray[0,:]))   
