...
fname = "RD.RFootMAO.dcm"
#%%
cd= os.getcwd()
path = os.path.join(cd,"RD.CN_XCAT.TBI_DVH_Exp.dcm")  
  

toto=dicom.dcmread(path)
...
while counter <  (coord[0]*coord[1]*coord[2]):#sum(coord):
  ...
  for l in range(len(line)):
     ...
     zslice=int((counter)/(sizel*sizec))
     j=int( (counter-zslice*sizel*sizec) /(sizec))
     i=int( (counter-zslice*sizel*sizec) -(j*sizec))
     #print i,j,k
...
toto.PixelSpacing = [xpixelsize, ypixelsize]

#Manual adjustments for CN's funky dicom origin 
...
toto.PixelData=data.tobytes()

#Replace the Instant UID for the RD file
iUID=toto['SOPInstanceUID']
nUID = dicom.uid.generate_uid()
toto.SOPInstanceUID = nUID
#********************   TO  DO *****************
# take new name of DICOM RD file as an argument
#    DONE
#toto.SaveAs(sys.argv[3] + 'RD_TT.dcm')
toto.save_as(fname)
print("RD file written")
