...
try:
    vessel_df = pd.read_pickle ('C:\\Users\\cbnor\\Documents\\Full Body Flow Model Project\\MOArray_Coronary.pkl')
except:
    vessel_df = pd.read_pickle ('C:\\Users\\Cassidy.Northway\\Remote Git\\MOArray_Coronary.pkl')
...
class Artery(object):
    ...
    def impedance_weights(self, r_root, dt, T, tc, rc, qc, nu):
        ...
        r_root = r_root * rc
        dt_temp = 0.005 #0.005 #Was 0.0001
    ...
    def impedance(self, s, r_root,r_0, N_alpha, N_beta, table, rc, qc , nu):
        ...
        if pos in [1,5,9,19,24,26,27,28,29,30]:
            r_min = 0.01/rc
        elif pos in [6,15,17,13,11,7]:
            r_min = 0.02/rc
        else:
            r_min = 0.03 /rc
        ...
        Z0 = Artery.singleVesselImpedance(self, ZL, s ,r_0 , rc, qc , nu, 50 )
...
class ArteryNetwork(object):
    ...
    def __init__(self, rho, nu, p0, ntr, Re, k, dataframe, Z_term, r_min, rc):
        """
        ...
        self.setup_arteries(Re, p0, k, r_min, Z_term,rc)     
        
    def setup_arteries(self, Re, p0, k, r_min, Z_term, rc):
        """
        ...
        while self.t < self.tf:
            ...
            for artery in self.arteries:
               ...
               ############################################
                ...
                if artery.pos in [0]and it%25 == 0:
                      plt.figure()
...
T = 1 #s
tc = 1 #Normally 4 #s
dt = 0.000025 #normally 0.25e-5 #s
...
#for x in [0,2,8,12,18,22,24,26,28]:
    #intial_values[x] = 40
