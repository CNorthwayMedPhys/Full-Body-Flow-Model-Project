...
class Artery(object):
    ...
    def impedance(self, s, r_root,r_0, N_alpha, N_beta, table, rc, qc , nu):
        ...
        if pos in [1,13,14,19,20,27,32,34,35,36,37,38]:
            r_min = 0.01/rc
        elif pos in [6,10,15,17,21,23,25]:
            r_min = 0.02/rc
        else:
            r_min = 0.03 /rc
...
class ArteryNetwork(object):
    ...
    def outlet_st(artery, dt, t, p_term):
        ...
        U0_3 = artery.U0[:,-3] # m = M-2
        a_n, q_n = U0_1
        p_new = artery.f[-1] * (1 - np.sqrt(artery.A0[-1]/a_n)) + p_term #Intial guess for P
        U_np_mp = (U0_1 + U0_2)/2 +\
                 gamma * (-(artery.F(U0_1, j=-1) - artery.F(U0_2, j=-2))/artery.dx +\
                         (artery.S(U0_1, j=-1) + artery.S(U0_2, j=-2))/2)
        U_np_mm = (U0_2 + U0_3)/2 +\
        ...
        while self.t < self.tf:
            ...
            for artery in self.arteries:
               ...
               ############################################
                ...
                if artery.pos in [18] and it%100 == 0:
                      plt.figure()
                      plt.plot(artery.U0[0,:], label = str(artery.pos))
                      plt.legend()
                      plt.title('After')
