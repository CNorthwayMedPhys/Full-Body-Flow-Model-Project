...
def runSim(lrr_values):    
    ...
    class Artery(object):
        ...
        def __init__(self, pos, Ru, Rd, lam, k, Re, p0, r_min, Z_term, rc, alpha, beta):
            """
            Artery constructor.
            """
            self._pos = pos
        ...
        def impedance_weights(self, r_root, dt, T, tc, rc, qc, nu):
            ...
            r_root = r_root*rc
            dt_temp = 0.05 #1e-3 #0.005 #Was 0.0001
        ...
        def initial_conditions(self, u0, dt, dataframe, lrr, T, tc,rc,qc, nu):
            """
        ...
        def factor(self):
            """
            Stored lrr tuning factor for structured tree values
            """
            return self._factor
    ...
    class ArteryNetwork(object):
        ...
        def __init__(self, rho, nu, p0, ntr, Re, k, dataframe, Z_term, r_min, rc, alpha, beta):
            """
            ...
            self.setup_arteries(Re, p0, k, r_min, Z_term,rc, alpha, beta)     
            
        def setup_arteries(self, Re, p0, k, r_min, Z_term, rc, alpha, beta):
            """
            ...
            for i in range(0,len(self.dataframe)):
                ...
                self.arteries[i] = Artery(i, Ru, Rd, lam, k, Re, p0, r_min, Z_term , rc, alpha, beta)
        ...
        def initial_conditions(self, intial_values, dataframe, lrr_factors, rc,qc):
            ...
            for artery in self.arteries:
                ...
                u0 = intial_value
                try: 
                    lrr = lrr_array[np.where(lrr_array[:,0]==(index)),1][0][0]
                except:
                    lrr = 0
        ...
        def bifurcation(parent, d1, d2, dt):
            ...
            while k < 1000:
                ...
                try:
                    ...
                    plt.figure()
                    plt.plot(d1.U0[1,:], label = str(d1.pos) + 'q')
                    plt.legend()
                    
                    plt.figure()
                    plt.plot(d2.U0[1,:], label = str(d2.pos) + 'q')
                    plt.legend()
                    
                    plt.figure()
                    plt.plot(parent.U0[0,:], label = str(parent.pos) + 'a')
                    plt.legend()
                   
                    plt.figure()
                    plt.plot(d1.U0[0,:], label = str(d1.pos) + 'a')
                    plt.legend()
                ...
                for artery in self.arteries:
                   ...
                   ############################################
                    ...
                    if end_condition != 'ST':
                        ...
                        flag, x_out = ArteryNetwork.bifurcation(artery, d1, d2, self.dt)
                        if flag == 1:
                    ...
                    if artery.pos in [17] and it%500 == 0:
                          plt.figure()
                          plt.plot(artery.U0[0,:], label = str(artery.pos))
                          plt.legend()
                          plt.title('After')
    ...
    lrr_array = np.vstack([intial_guess[:,0],lrr_values]) 
    lrr_array = np.transpose(lrr_array)
    #%% Run simulation
    ...
    an = ArteryNetwork(rho, nu, p0, ntr, Re, k, dataframe, Z_term, r_min, rc, alpha, beta)
    an.mesh(dx)
    an.set_time(dt, T, tc)
    an.initial_conditions(intial_value, dataframe,lrr_array, rc,qc)
    ...
    file_name = 'Artery_Array_Cousins'
    try:
