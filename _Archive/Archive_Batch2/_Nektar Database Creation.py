...
#Import excel sheet
try:
    arteries_sheet = pd.read_excel('C:\\Users\\Cassidy.Northway\\RemoteGit\\FlowTracker_Nektar.xlsx', sheet_name = 0)
    #veins_sheet = pd.read_excel('C:\\Users\\Cassidy.Northway\\GitRemoteRepo\\FlowTracker.xlsx', sheet_name = 1)
except:
    arteries_sheet = pd.read_excel('C:\\Users\\cbnor\\Documents\\Full Body Flow Model Project\\FlowTracker_Nektar.xlsx', sheet_name = 0)
...
df = pd.DataFrame(columns=['Name', 'Length', 'End Condition']) 
...
for index in range(0,sheet.shape[0]):
    ...
    if seg_tag == False:
        ...
        lam_value = tot_dist /10 #to convert to cm
        
        #Add to the dataframe
        new_row = {'Name' : seg_name, 'Length': lam_value, 'End Condition': final_cnd }
    ...
    if seg_tag == True:
        ...
        for i in range(0,len(branches)):
            branch_row = sheet[sheet['Filename'].str.fullmatch(branches[i])].index.values[0]
        ...
        for i in range(0,len(seg_df)+1):
            if i != len(seg_df):
                ...
                end_cnd = [file_name + '_' + str(i+1), seg_df.at[i, 'Branch Name' ]+'_0' ]        
                ...
                for i in range(0,np.shape(center_array)[0]-1):
                    dist = np.linalg.norm(center_array[i,:] - center_array[i+1,:])
                    tot_dist = tot_dist + dist    
                lam_value = tot_dist / 10 
                
                #Add to the dataframe
                new_row = {'Name' : sub_name, 'Length': lam_value, 'End Condition': end_cnd }
                df.loc[len(df)] = new_row
                ...
                for i in range(0,np.shape(center_array)[0]-1):
                    dist = np.linalg.norm(center_array[i,:] - center_array[i+1,:])
                    tot_dist = tot_dist + dist    
                lam_value = tot_dist / 10 #convert to cm
...
for i in range (0,len(df)):
    ...
    if end_condition != 'ST' and len(end_condition)==1:
        ...
        name = df.at[i,'Name']
        lam_value = (df.at[i,'Length']+df.at[index, 'Length'])
        new_end_condition = df.at[index,'End Condition']
        new_row = {'Name' : name, 'Length': lam_value,'End Condition': new_end_condition }
        df.loc[len(df)] = new_row
...
df_ordered = pd.DataFrame(columns=['Name', 'Length', 'End Condition']) 
...
df_ordered.to_pickle('NektarArteries.pkl')       
