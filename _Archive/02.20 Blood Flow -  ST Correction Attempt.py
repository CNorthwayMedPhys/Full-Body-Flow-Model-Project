...
class Artery(object):
    ...
    def impedance_weights(self, r_root,dt, T, tc):
        ...
        z_n = np.zeros(int(((T/dt)*tc)+10), dtype = np.complex_)
    ...
    def singleVesselImpedance(self,ZL, s_range, r_0):
        ...
        for s in s_range:
            if s == 0:
                Z0[i] = ZL[i] + (2*(gamma +2)*mu* self.lrr) / (np.pi * r_0**3)
    ...
    def initial_conditions(self, u0, dt, dataframe, T, tc):
        ...
        if  dataframe.at[self.pos,'End Condition'] == 'LW':
            ...
            self._zn = zn
            self._Qnk = np.zeros(int((T/dt)*tc+1))
        else:
            self._zn = 0
            self._Qnk = 0
...
class ArteryNetwork(object):
    ...
    def __init__(self, rho, nu, mu, p0, ntr, Re, k, dataframe, Z_term, alpha, beta, r_min, lrr):
        """
    ...
    def solve(self, q_in, out_bc, out_args):
        ...
        while self.t < self.tf:
            ...
            for artery in self.arteries:
                ...
                #End simulation if non-real values genererated
...
time step size." % (self.t))
                    ...
                    #plt.show
                    #plt.title('Artery Position 1')

            # Here we save the Q values at the end of the truncated arteries (x=L) at each the current time step 
                if end_condition == 'LW':
                    k_array = np.arange(0,self.t+self.dt,self.dt) #actual range [0,t]
                    n_value = np.size(k_array)
                    artery.Qnk[n_value-1] = artery.U0[1,-1]
...
alpha = 0.88
beta =0.66
lrr = 50 #Hard coded in the artery currently
