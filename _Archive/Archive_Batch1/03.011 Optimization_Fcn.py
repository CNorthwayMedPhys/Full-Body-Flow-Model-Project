...
import numpy as np
import os
from runSimulationCorrected_rmin import runSimulation
import warnings
...
def ModelError(lambda_vals):
    ...
    #Run simulation
    print('Current r_min ' + str(lambda_vals))
    try:
        runSimulation(lambda_vals)
        #Prep for error calcs !HARD CODED!
        v_number = [0] 
        EU = np.zeros ([50, 1]) #!HARD CODED! 2 = num vessles, 50 = number time steps
        EP = np.zeros ([50, 1])
        E = np.zeros([2,]) #4 = num vessels * 2 (for EU and EP)
