...
class Artery(object):
    ...
    def impedance_weights(self, r_root, dt , T, tc):
        ...
        eta = acc**(1/(2*N))
        empty_table = {}
        m = np.arange(0,(2*N),1) #actual [0:2N-1] the size of 2N 
        zeta = eta * np.exp(1j*np.pi*m/N)
        ...
        weighting = np.concatenate (([1], 2*np.ones((2*N)),[1]))/ (4 * N)
        test = Z_impedance * np.exp(-1j * np.pi * m / N )
        for n in range(0,N+1): # actual range [0,N]
            z_n[n] = np.sum(Z_impedance * np.exp(-1j * np.pi * m * n / N ))
            z_n[n] = (eta**(-n)/(2*N)) * z_n[n]
        z_n = np.real(z_n)
        ...
        for s in s_range:
            if s == 0:
                ...
                #except FloatingPointError:
                    ...
                    #print(np.tanh(L/d_s))
                    #print('end')
                    #print(ZL[i] + ((np.tanh(L/d_s))/(s*d_s*C)))
