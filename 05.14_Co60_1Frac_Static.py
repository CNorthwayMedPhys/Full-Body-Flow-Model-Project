...
Tss =  T#round(400*T,3) #Time to reach Steady State (s) #Needs to be a round number!!!
Tfield = round((0.45*1) * 60, 3) #round((0.45*15) * 60, 3) #Time per field (s)
...
class Location (object):
    ...
    def IntEventFreq (self,number, xx):
            if number == 11:
                ...
                number = 15    
            if xx == 'AP':
                EventFreqArrayPath = os.path.join(cd,compartment_AP,str(number)+'eventTrace2msRes.npy')
            else:
               EventFreqArrayPath = os.path.join(cd,compartment_PA,str(number)+'eventTrace2msRes.npy')    
