...
dst = 0.002 #Dose sample time step size (s)
...
#Dose file locations 
cd = os.getcwd()
dose_filename_AP = "timeStepQuery\\XCAT_AP_2ms15E9.npy"
...
def DoseRateSampler(EventFreqArray, time):
    if time < EventFreqArray[0,0]:
        DoseRate = 0
    elif time >= EventFreqArray[0,-1] + 0.5:
        DoseRate = 0
    else:    
...
plt.title('Single AP Fields 0.002s')
plt.legend()
