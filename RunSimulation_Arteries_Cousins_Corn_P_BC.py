...
class ArteryNetwork(object):
    ...
    @staticmethod
    def outlet_p(artery, dt, t, p_term ):
        """
        ...
        U0_2 = artery.U0[:,-2]
        a_n, q_n = U0_1
        Qnk = np.concatenate((Qnk_array , [q_n]))
        p_out = np.sum(zk_array*Qnk) + p_term
    ...
    def outlet_st(artery, dt, t, p_term):
        ...
        while k < 1000:
            p_old = p_new
            q_out = (p_old - p_term - np.sum(zk_array[1:]*Qnk_array))/zk_array[0]
            a_out = a_n - theta * (q_out - U_mm[1])
            p_new = artery.p(a_out, j=-1) 
            if abs(p_old - p_new) < 1e-7:                
                break
            ...
            for artery in self.arteries:
               ...
               ############################################
                ...
                if end_condition == 'ST':
                    ...
                    if out_bc == 'p':
                        U_out = ArteryNetwork.outlet_p(artery, self.dt,self.t, p_term)
                    elif out_bc == 'ST':
