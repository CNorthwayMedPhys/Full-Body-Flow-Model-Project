...
INSTRUCTIONS: turn this script into an executable (e.g. g++ -Wall put_lead_on_top -o lead), then with the executable, enter four arguments:


./lead newID ("AP or "PA") shift_y
...
lungcomps.egsphant (an egsphant produced in Matlab for the compensators)
plastic_VC_TBI(newID)_(APorPA).egsphant, e.g. plastic_VC_TBI01_PA.egsphant (produced in put_plastic_on_patient)

Output file will be:  lead_plastic_VC_TBI(newID)_(APorPA).egsphant, e.g. plastic_VC_TBI01_PA.egsphant

Shift_y calculation: (SLIGHTLY DIFFERENT FROM PLASTIC) (y_bed) - (44.8-n(3.7)) - (thickness) where n is the number of styro - use the same value as plastic but subtract the thickness of the compensators.
...
//argv[1]= ID
//argv[2]= AP or PA
//argv[3]= shift_y for lead (entered as a negative value)
  char egsphantFileName2[256];
  char outputegsphantFileName[256];

  strcpy(egsphantFileName2,"plastic_");
  ...
  strcpy(outputegsphantFileName,"lead_plastic_");
...
char* egsphantFileName1="lungcomps.egsphant";
...
for(int i=0;i<20;i++)
  ...
  //*********************
  //Shift_y calculation: (y_bed) - (44.8-n(3.7)) - (thickness of comps) where n is the number of styro
  // y_bed is the bed y-coordinate, obtained from the egsphant in dosxyzshow
  //*********************

  float shift_y=atof(argv[3]); // Hand calc where to shift the lung block
