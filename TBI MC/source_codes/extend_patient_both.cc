...
int readPhantomBoundariesExtended(FILE *istrm, int nBounds, float *bounds)
{
  ...
  printf("\n ADDING THESE NEW COORDINATES BOTH DIR \n");
  ...
  for(int k=extend_pixel+nBounds;k<extend_pixel+nBounds+extend_pixel_front;k++) {
    bounds[k]=bounds[k-1]+pixel_size;
    //printf("3.7%f\t",bounds[k]);
  }
  return(OK);
}
/* *********************************************************************** */
...
//TT
  ...
  p->mednum = (int *)calloc(p->x_num*p->y_num*(p->z_num+extend_pixel+extend_pixel_front),sizeof(int));
  if(p->mednum == NULL)
  {
  ...
  p->densval = (float *)calloc(p->x_num*p->y_num*(p->z_num+extend_pixel+extend_pixel_front),sizeof(float));
  if(p->densval == NULL)
  {
  ...
  //now adding the extended_front part at the end
  for (k=extend_pixel+p->z_num;k<extend_pixel+p->z_num+extend_pixel_front;k++) 
    {
      for (j=0;j<p->y_num;j++) 
	{
          for (i=0;i<p->x_num;i++) 
  ...
  // now adding the extended part at the end
  for (k=extend_pixel+p->z_num;k<extend_pixel+p->z_num+extend_pixel_front;k++) 
    {
      for (j=0;j<p->y_num;j++) 
	{
 ...
 */
  ...
  p->z_num+=extend_pixel+extend_pixel_front;
...
//char* egsphantFileName="TBIFilter.egsphant";
  ...
  strcpy(egsphantFileName,argv[1]);
  strcat(egsphantFileName,".egsphant");
  strcpy(outputegsphantFileName,"extended2_");
  strcat(outputegsphantFileName,argv[1]);
  strcat(outputegsphantFileName,".egsphant");


//CHANGE NAMES HERE
//char *egsphantFileName="lead_plastic_VC_TBI02_AP.egsphant";
//char *outputegsphantFileName="extended2_lead_VC_TBI02_AP.egsphant";
