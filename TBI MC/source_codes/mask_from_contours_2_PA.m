...
xmin = 23.25; %absolute value of xmin
xmax = 21.5;
zmin = 5.25; %absolute value of zmin (is negative; look at egsphant)
zmax = 38.75;
t2zscaled = 14.4;

zoffset = 0.0; %when the scans aren't on 0.25 multiples; add up to next 0.25 from zmax

xdim = '179';  %from egsphant; same dimensions
zdim = '176';  %from egsphant; same dimensions
compthick = '0.2000'; %from spreadsheet - in centimeters
traythick = '0.6000';

zsize = 1050;
xsize = 400;
t2zscaled = t2xscaled*1.397;
...
xLadd = round((xmin/xres)-xsize);
xRadd = round((xmax/xres)-xsize);
...
xq = 0:xresphant:xmin;
%zq = (0.25-zoffset:zresphant:zmax)'; %z grid needs to be a column
zq = (0:zresphant:zmax)';
...
finallungarrayL = padarray(finallungarrayL,[(zmin-zoffset)/0.25 0],0,'pre');
...
xqR = 0:xresphant:xmax;
finallungarrayR = interp2(xssR,zss,double(lungmaskpaddedR),xqR,zq,'nearest');


finallungarrayR = finallungarrayR(1:end-1,1:end-1);

finallungarrayR = padarray(finallungarrayR,[(zmin-zoffset)/0.25 0],0,'pre');

if zoffset > 0 %because it samples one less point; need to pad another
   finallungarrayL = padarray(finallungarrayL,[1 0],0,'pre'); 
   finallungarrayR = padarray(finallungarrayR,[1 0],0,'pre');
end

zq = [(-zmin:zresphant:-0.25)'; zq];
%zq = [(-zmin:zresphant:-zoffset)'; zq]; %for extending down to the bottom of patient scan
%xqR = xq*-1;

%size(finallungarrayL)
%size(finallungarrayR)

finallungarrayL = fliplr(finallungarrayL);

bothlungs = cat(2,finallungarrayL,finallungarrayR);
...
% adjust thickness here!!!!!!!!!!!!!!!!!!!!!! normal is 0.6000
